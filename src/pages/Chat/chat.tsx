                          ...
                          <span className="block ml-2 text-sm text-gray-600 w-[90%] overflow-hidden whitespace-nowrap text-ellipsis" style={{
                              fontWeight:
                                item.last_message.isCustomer || item.seen
                                  ? "normal"
                                  : "bold",
                            }}>{item.last_message.message}</span>
                        </div>
                      </a>
                    ))
                  : chat?.map((item: any, index: number) => (
          ...
          <div className="w-full">
            <div className="relative flex items-center p-3 border-b border-gray-300">
              <img className="object-cover w-10 h-10 rounded-full"
                ...
                {user?.role === "Customer" ? (
                  <div>
                    <span className="block ml-2 font-bold text-gray-600">{message?.saler?.name}</span>
                    <span className="block ml-2 font-bold text-gray-600">{message?.saler?.email}</span>
                  </div>
                    
                  ) : (
                    <div>
                    <span className="block ml-2 font-bold text-gray-600">{message?.customer?.name}</span>
                    <span className="block ml-2 font-bold text-gray-600">{message?.customer?.email}</span>
                  </div>
                  )}
                  {user?.role === "Customer" ? (
                    <button
            ...
            <div className="flex items-center justify-between w-full p-3 border-t border-gray-300">              
              ...
              <button>
                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-gray-500" fill="none" viewBox="0 0 24 24"
                  stroke="currentColor">
                  <path stroke-strokeLinecap="round" stroke-linejoin="round" stroke-width="2"
